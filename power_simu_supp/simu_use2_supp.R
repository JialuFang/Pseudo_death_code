...
S_death_fct1 <- function(t){1/(1+t/2)}
inv_S_death_fct1 <- function(x){(1 / x - 1)*2}

S_prog_fct2 <- function(t, lambda=1/4){
  exp(-(3/2+lambda)*t + 2*lambda*pmax(t-1, 0))
}
inv_S_prog_fct2 <- function(x, lambda=1/4){
  target_fct <- function(t, x, lambda){
    exp(-(3/2+lambda)*t + 2*lambda*pmax(t-1, 0)) -x
  }
  result <- uniroot(target_fct, lower = 0, upper = 1000, x = x, lambda = lambda)
  return(result$root)
}

S_death_fct2 <- function(t, t0=1){
  term1 <- 1 + t/2
  term2 <- 1 + pmin(t, 1 + t0/2) - pmin(t, 1 - t0/2)
  return(1 / (term1 * term2))
}
inv_S_death_fct2 <- function(x, t0=1){
  target_fct <- function(t, x, t0) {
    term1 <- 1 + t/2
    term2 <- 1 + pmin(t, 1 + t0/2) - pmin(t, 1 - t0/2)
    return(1 / (term1 * term2) - x)
  }
...
followup_para1 <- 2; followup_para2 <- 2; tau <- 1
run.fn <- function(i){
  set.seed(seeds[i])
  if (i%%20 == 0){
    print(i)
...
for (ii in 1:4){
  ...
  file.name <- paste0("./test_results/","Test_",samplesize,"_simu_use2_supp", ".Rdata")
