...
S_prog_fct2 <- function(t, lambda=1/2){
  lambda*(1/(1+3*t))+(1-lambda)*exp(-3/2*t)
}
inv_S_prog_fct2 <- function(x, lambda=1/2){
  target_fct <- function(t, x, lambda){
    lambda*(1/(1+3*t))+(1-lambda)*exp(-3/2*t) -x
  }
...
for (ii in 1:4){
  ...
  file.name <- paste0("./test_results/","Test_",samplesize,"_simu_use3_supp", ".Rdata")
