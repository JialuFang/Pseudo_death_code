...
run.fn <- function(i){
  ...
  #pseudo death Z
  pseu.res.Z <- pseudeath.test.Z(data1, data2, tau, B=B)
  
  #pseudo death per
  pseu.res.per <- pseudeath.test.per(data1, data2, tau, B=B)
  
  # pseudo death wald
  pseu.res.wald <- pseudeath.test.wald(data1, data2, tau, ndots=4, B=B)
...
B_list <- c(500, 200, 200, 200)
for (ii in 1:4){
  nsimu <- 1000
  tau <- 2; samplesize <- samplesize_list[ii]; B <- B_list[ii]
  test.res <- list()
  seeds <- c(1:nsimu)
  t <- system.time({
    test.res <- mclapply(1:nsimu, run.fn, mc.cores=40)
  })
  file.name <- paste0("./test_results/","H0_Test_",samplesize,"_simu3", ".Rdata")
